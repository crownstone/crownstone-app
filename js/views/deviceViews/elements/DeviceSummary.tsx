  ...
  _getMenuIcons() {
    return (
      <View style={{width:screenWidth, height:screenWidth/5, marginTop:10, marginBottom:10, alignItems:'center', flexDirection:'row'}}>
        ...
        <View style={{flex:1}} />
        <DeviceMenuIcon icon={'ios-settings'} selected={false} callback={() => {}} />
        <View style={{flex:1}} />
      </View>
    )
  }
...
export function DeviceMenuIcon(props) {
  ...
  let innerPart = (
    <View style={{width:size, height:size, borderRadius:0.5*size, borderWidth: borderWidth, borderColor: colors.darkBackgroundOverlay.rgba(0.8)}}>
      <View style={{
        width:innerSize,
        height:innerSize,
