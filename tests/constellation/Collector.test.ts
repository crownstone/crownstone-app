...
test("Collect handles", async () => {
  ...
  let { stone: stone1, handle:handle1 } = addStone({locationId: location1.id}, meshId);
  let { stone: stone2, handle:handle2 } = addStone({locationId: location1.id}, meshId);
  let { stone: stone3, handle:handle3 } = addStone({locationId: location2.id}, meshId);
  let { stone: stone4, handle:handle4 } = addStone({locationId: location2.id}, otherMeshId);
  let { stone: stone5, handle:handle5 } = addStone({locationId: location1.id}, otherMeshId);
