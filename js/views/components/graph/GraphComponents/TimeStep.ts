 ...
 */
export function TimeStep(start, end, minimumStep = 200) {
  this.moment = moment;
  // variables
  ...
  this.format = TimeStepFORMAT; // default formatting
}

// Time formatting
const TimeStepFORMAT = {
  minorLabels: {
    millisecond:'SSS',
    second:     's',
    minute:     'HH:mm',
...
TimeStep.prototype.setRange = function(start, end, minimumStep) {
  if (!(start instanceof Date) || !(end instanceof Date)) {
    console.error("No legal start or end date in method setRange")
  }
 ...
 */
TimeStep.prototype.snap = function(date, scale, step) {
  let clone = moment(date);

  if (scale == 'year') {
    let year = clone.year() + Math.round(clone.month() / 12);
...
TimeStep.prototype.isMajor = function() {
  ...
  let date = this.moment(this.current);
  switch (this.scale) {
    case 'millisecond':
...
TimeStep.prototype.getLabelMinor = function(date) {
  ...
  let format = this.format.minorLabels[this.scale];
...
TimeStep.prototype.getLabelMajor = function(date) {
  ...
  let format = this.format.majorLabels[this.scale];
  return (format && format.length > 0) ? this.moment(date).format(format) : '';
};
