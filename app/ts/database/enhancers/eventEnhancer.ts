...
function checkAction(action : DatabaseAction, affectedIds, oldState, newState) {
  let eventStatus : DatabaseChangeEventData = {};

  if (action.locationId) { affectedIds.locationIds[action.locationId] = true; affectedIds.id[action.locationId] = true; }
  ...
  switch (action.type) {
    ...
    case 'REMOVE_ALL_PROCESSED_FINGERPRINTS':
      eventStatus.removeAllProcessedFingerprint = affectedIds; break;
    ...
    case "CHANGE_DEV_SETTINGS":
      eventStatus.changeUserDeveloperStatus = affectedIds; break;
    case "SET_LOGGING":
    ...
    case "REMOVE_ALL_SCENES":
      eventStatus.changeScenes = affectedIds;
    ...
    case "CLOUD_EVENT_REMOVE_FINGERPRINTS":
    ...
    case "FINISHED_REMOVE_FINGERPRINTS":
