...
export class LocalizationSetupStep2 extends Component<any, any> {
  ...
  render() {
    ...
    return (
      ...
      >
        <Text style={{fontSize: 23, fontWeight: 'bold', color: colors.menuBackground.hex, padding:15}}>{ Languages.text("LocalizationSetupStep2", "The_Next_Step")() }</Text>
        <Image source={require('../../images/localizationExplanation.png')} style={{width:0.6*screenWidth, height:0.6*screenWidth}}/>
        <Text style={{fontSize: 13, color: colors.blue.hex, textAlign:'center'}}>{ Languages.text("LocalizationSetupStep2", "You_can_now_teach__when_y")(ai.name,ai.name) }</Text>
        <View style={{flex:1}}/>
        <Text style={{fontSize:14, fontWeight:'bold', color: colors.blue.hex, textAlign:'center'}}>{ Languages.text("LocalizationSetupStep2", "Once_youre_ready__tap_one")() }</Text>
        <View style={{flex:1}}/>
        <TouchableOpacity onPress={() => {this.setState({visible:false});}} style={[styles.centered,{width:0.4*screenWidth, height:36, borderRadius:18, borderWidth:2, borderColor:colors.blue.rgba(0.25), marginBottom:10}]}>
          <Text style={{fontSize: 13, color: colors.blue.hex}}>{ Languages.text("LocalizationSetupStep2", "OK")() }</Text>
        </TouchableOpacity>
      </OverlayBox>
    );
  }
