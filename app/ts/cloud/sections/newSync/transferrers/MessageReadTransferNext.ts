...
export const MessageReadTransferNext : TransferMessageTool<MessageStateData, MessageStateData, cloud_MessageState, cloud_MessageState_settable> = {
  ...
  getCreateLocalAction(localSphereId: string, localMessageId: string, data: Partial<MessageStateData>) : {id: string, action: DatabaseAction } {
    let newId = 'read';
    let action : DatabaseAction = {type:"MARK_AS_READ", sphereId: localSphereId, messageId: localMessageId, messageStateId: newId, data: data };
  ...
  getUpdateLocalCloudIdAction(localSphereId: string, localMessageId: string, localItemId: string, cloudId: string) : DatabaseAction {
    return {type:"UPDATE_READ_MESSAGE_CLOUD_ID", sphereId: localSphereId, messageId: localMessageId, messageStateId: localItemId, data: {cloudId}};
  },
  ...
  getUpdateLocalAction(localSphereId: string, localMessageId: string, localItemId: string, data: Partial<MessageStateData>) : DatabaseAction {
    return {type:"UPDATE_READ_MESSAGE", sphereId: localSphereId, messageId: localMessageId, messageStateId: localItemId, data: data }
  },


  getRemoveFromLocalAction(localSphereId: string, localMessageId: string, localItemId: string) : DatabaseAction {
    return {type:"REMOVE_MESSAGE_READ", sphereId: localSphereId, messageId: localMessageId, messageStateId: localItemId }
  },
  ...
  async createOnCloud(localSphereId: string, localMessageId: string, data: MessageStateData) : Promise<cloud_MessageState> {
    ...
    core.store.dispatch(MessageReadTransferNext.getUpdateLocalCloudIdAction(localSphereId, localMessageId, data.id, cloudItem.id));
    return cloudItem;
  },
