...
export class SettingsStoneBleDebug extends Component<any, any> {
  ...
  _getItems() {
    ...
    items.push({label: Languages.label("SettingsStoneBleDebug", "iBeacon_UUID___niBeacon_M")(this._ibeaconUuid.toUpperCase(),this._major,this._minor), type: 'explanation', style: { paddingTop:0, paddingBottom:0 } });
    items.push({label: Languages.label("SettingsStoneBleDebug", "Latest_iBeacon_data_")(), type: 'largeExplanation', style:{paddingTop:0}});
    items.push({__item:
      <View style={{backgroundColor: colors.white.hex, minHeight: 100}}>
        <Text style={{padding:15, color: new Date().valueOf() - this.state.ibeaconTimestamp > 10000 ? colors.gray.hex : colors.black.hex, fontSize:12}}>{ Languages.text("SettingsStoneBleDebug", "No_Data")(this.state.ibeaconPayload) }</Text>
      </View>
    });
    items.push({label: Languages.label("SettingsStoneBleDebug", "Time_received__no_data")(this.state.ibeaconTimestamp,new Date(this.state.ibeaconTimestamp)), type: 'explanation', below: true});

    items.push({label: Languages.label("SettingsStoneBleDebug", "Green_Background_means_ex")(), type: 'largeExplanation'});

    items.push({label: Languages.label("SettingsStoneBleDebug", "Latest_Direct_Advertiseme")(), type: 'largeExplanation'});
    items.push({__item:
        <View style={{backgroundColor: this.state.directAdvertisementStateExternal ? colors.green.rgba(0.1) : colors.white.hex, minHeight: 100}}>
          <Text style={{padding:15, color: new Date().valueOf() - this.state.directAdvertisementTimestamp > 10000 ? colors.gray.hex : colors.black.hex, fontSize:12}}>{ Languages.text("SettingsStoneBleDebug", "No_Data")(this.state.directAdvertisementPayload) }</Text>
        </View>
    });
    items.push({label: Languages.label("SettingsStoneBleDebug", "Time_received__no_data")(this.state.directAdvertisementTimestamp,new Date(this.state.directAdvertisementTimestamp)), type: 'explanation', below: true});


    items.push({label: Languages.label("SettingsStoneBleDebug", "Latest_Applied_Advertisem")(), type: 'largeExplanation'});
    items.push({__item:
        <View style={{backgroundColor: this.state.advertisementStateExternal ? colors.green.rgba(0.1) : colors.white.hex, minHeight: 100}}>
          <Text style={{padding:15, color: new Date().valueOf() - this.state.advertisementTimestamp > 10000 ? colors.gray.hex : colors.black.hex, fontSize:12}}>{ Languages.text("SettingsStoneBleDebug", "No_Data")(this.state.advertisementPayload) }</Text>
        </View>
    });
    items.push({label: Languages.label("SettingsStoneBleDebug", "Time_received__no_data")(this.state.advertisementTimestamp,new Date(this.state.advertisementTimestamp)), type: 'explanation', below: true});
