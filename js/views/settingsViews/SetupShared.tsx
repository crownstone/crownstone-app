  ...
  render() {
    return <TouchableOpacity onPress={this.props.onPress} >
      <View style={{paddingRight:20, flexDirection:'row', height:30}}>
        <Text style={[setupStyle.buttonText, {color:this.props.dark ? colors.menuBackground.hex : '#fff'}]}>{ Languages.text("SetupShared", "Next")() }</Text>
...
export class StartButton extends Component<any, any> {
  render() {
    return <TouchableOpacity onPress={this.props.onPress} >
      <View style={{paddingRight:20, flexDirection:'row', height:30}}>
        <Text style={[setupStyle.buttonText]}>{ Languages.text("SetupShared", "Start_setup")() }</Text>
  ...
  render() {
    return <TouchableOpacity onPress={this.props.onPress} >
      <View style={{paddingLeft:20, flexDirection:'row', height:30}}>
        ...
        <Text style={[setupStyle.buttonText,{fontWeight:'300'}]}>{ Languages.text("SetupShared", "Skip")() }</Text>
      </View>
    </TouchableOpacity>
  }
}
...
export class CancelButton extends Component<any, any> {
  render() {
    return <TouchableOpacity onPress={this.props.onPress} >
      <View style={{paddingLeft:20, flexDirection:'row', height:30}}>
        <Icon name="ios-remove-circle-outline" size={30} color={'#fff'} style={{position:'relative', top:-2, paddingRight:8}} />
        <Text style={[setupStyle.buttonText,{fontWeight:'300'}]}>{ Languages.text("SetupShared", "Cancel")() }</Text>
      </View>
    </TouchableOpacity>
  }
}